...
class ICarl(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        if args.use_original_icarl_transform:
            ...
            'seq-inaturalist',
            'seq-inaturalist-300'
        }, 'The original iCaRL transform is only available for CIFAR-100 variants, CUB-200, and iNaturalist.'
        ...
        if getattr(self.args, 'inference_only', 0) and getattr(self.args, 'loadcheck', None):
            try:
                from utils.checkpoints import mammoth_load_checkpoint
                mammoth_load_checkpoint(self.args, self, ignore_classifier=False)
            except Exception as err:
                logging.warning('Failed to load checkpoint in inference-only mode: %s', err)
                if getattr(self, 'old_net', None) is None:
    ...
    def forward(self, x):
        if self.class_means is None:
            if hasattr(self.buffer, 'examples') and len(self.buffer) > 0:
    ...
    def observe(self, inputs, labels, not_aug_inputs, logits=None, epoch=None):
        ...
        if getattr(self, '_concept_reg', None) and (self.epoch_iteration % max(1, self.args.concept_units_freq) == 0):
            reg = self._concept_reg.penalty(self, inputs)
            # print(f"Concept regularizer: {reg}")
            if reg is not None:
                loss = loss + self.args.concept_units_lambda * reg
        loss.backward()
    ...
    def begin_task(self, dataset):
        if getattr(self.args, 'inference_only', 0):
            return
        if self.current_task == 0:
    ...
    def compute_class_means(self) -> None:
        ...
        if not hasattr(buffer, 'examples') or len(buffer) == 0:
            self.class_means = None
