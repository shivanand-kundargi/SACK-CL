...
class CodaPrompt(ContinualModel):
    ...
    def get_parser(parser) -> ArgumentParser:
        ...
        parser.add_argument('--weighted_gradient', type=int, default=0,
                            help='use concept-based per-class gradient scaling')
        return parser
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=0, grad_scales=None):
        labels = labels.long()
        self.opt.zero_grad()
        ...
        if grad_scales is not None and getattr(self.args, "weighted_gradient", 0):
            # Per-sample cross-entropy scaled by grad_scales
            per_sample_ce = F.cross_entropy(logits, labels, reduction='none')
            if grad_scales.shape[0] != per_sample_ce.shape[0]:
                raise ValueError(f"grad_scales shape {grad_scales.shape} does not match batch size {per_sample_ce.shape[0]}")
            loss_ce = (per_sample_ce * grad_scales).mean()
        else:
            ...
            if getattr(self, '_concept_reg', None) and (self.epoch_iteration % max(1, self.args.concept_units_freq) == 0):
                ...
                if reg is not None:
                    loss_ce = loss_ce + self.args.concept_units_lambda * reg
