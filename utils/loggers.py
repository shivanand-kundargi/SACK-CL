...
class Logger:
    ...
    def add_bwt(self, results, results_mask_classes):
        ...
        if any(len(row) < expected_len for row in results):
            logging.warning('Skipping BWT computation due to incomplete result matrix.')
            return self.bwt, self.bwt_mask_classes
        self.bwt = backward_transfer(results)
        if self.setting == 'class-il':
            if results_mask_classes and not any(len(row) < expected_len for row in results_mask_classes):
...
def log_extra_metrics(args, metric: float, metric_mask_class: float, metric_name: str, t: int, prefix="RESULT"):
    ...
    Args:
        ...
        prefix: The prefix for the metrics (default="RESULT").
    """
    if metric is None and metric_mask_class is None:
        return
    if metric is None:
        print(f'{metric_name}: [Task-IL]: {metric_mask_class:.2f}', file=sys.stderr)
        log_dict = {f'{prefix}_task_{metric_name}': metric_mask_class, 'Task': t}
    elif metric_mask_class is None:
        print(f'{metric_name}: [Class-IL]: {metric:.2f}', file=sys.stderr)
        log_dict = {f'{prefix}_class_{metric_name}': metric, 'Task': t}
    else:
