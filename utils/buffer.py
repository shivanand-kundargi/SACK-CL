    ...
    if self.current_task > 0:
        ...
        def _ensure_object_array(container):
            if isinstance(container, np.ndarray) and container.dtype != object:
                return container.astype(object)
            if isinstance(container, list):
                obj_arr = np.empty(len(container), dtype=object)
                for idx, item in enumerate(container):
                    obj_arr[idx] = item
                return obj_arr
            return container

        def _refold_as_object(x, channel_last=True):
            imgs = (x.cpu() * 255.0).clamp(0, 255).to(torch.uint8)
            if channel_last:
                imgs = imgs.permute(0, 2, 3, 1)
            else:
                imgs = imgs.squeeze(1)
            imgs_np = imgs.numpy()
            wrapper = np.empty(imgs_np.shape[0], dtype=object)
        ...
        if not need_aug:
            ...
            if data_shape == 3:
                ...
                def refold_transform(x):
                    return (x.cpu() * 255).squeeze(1).to(torch.uint8)
            else:
                dataset.train_loader.dataset.data = _ensure_object_array(data_container)
                data_concatenate = np.concatenate

                if val_set_split > 0 and hasattr(self, 'val_dataset'):
                    self.val_dataset.data = _ensure_object_array(self.val_dataset.data)

                def refold_transform(x):
                    return _refold_as_object(x, channel_last=True)
