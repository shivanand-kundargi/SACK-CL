...
def add_initial_args(parser) -> ArgumentParser:
    ...
    parser.add_argument('--concept_units_lambda', type=float, default=0.1,
                        help='Weight for the unit activation regularizer.')
    parser.add_argument('--concept_units_layer', type=str, default='last',
                        help='Layer label used by CLIP-Dissect (e.g., last, layer4).')
    parser.add_argument('--concept_units_percentile', type=float, default=80.0,
                        help='Keep units with similarity >= this percentile.')
    parser.add_argument('--concept_units_stats_dir', type=str, default=None,
                        help='Directory of saved activations. Defaults to SACK/saved_activations_{model}_{dataset}.')
    parser.add_argument('--concept_units_freq', type=int, default=1,
                        help='Apply the penalty every N steps.')
...
def add_management_args(parser: ArgumentParser) -> None:
    ...
    mng_group.add_argument('--use_uncertainty_sampling', type=binary_to_boolean_type, default=0,
                           help='Enable uncertainty-based sampling for tasks that support concept guidance.')
    mng_group.add_argument('--uncertainty_eval_batch_size', type=int, default=None,
                           help='Batch size used to compute uncertainty scores (defaults to the training batch size when omitted).')
    mng_group.add_argument('--log_perf_metrics', type=binary_to_boolean_type, default=0,
                           help='Log per-task runtime metrics (training time, evaluation time, GFLOPs). Adds profiling overhead.')
