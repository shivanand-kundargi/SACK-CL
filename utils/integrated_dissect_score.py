...
class IcarlDissectandScore:
    ...
    def dissect(self, save_dir: str):
        ...
        if not os.path.exists(f"{PATHS_CONFIG['activations_path']}/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}"):
            os.mkdir(f"{PATHS_CONFIG['activations_path']}/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}")
        ...
        # os.mkdir(save_path)
        save_path = f"{PATHS_CONFIG['activations_path']}/results_task{self.experience_number+1}_{self.args.model}_{self.args.dataset}"
    ...
    def scoring_function(self,clip_model,filtered_concept_set, next_exp_concept_set_path, indices, device):
        ...
        #filter out redundant concepts and corresponding scores, consider only those concepts which are not redundant
        filtered_concept_set =  set([desc for desc, sim in zip(self.outputs["description"], similarity_scores) if sim > threshold])
