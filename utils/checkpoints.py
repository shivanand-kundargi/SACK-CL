...
def _load_mammoth_model(dict_keys, model: torch.nn.Module, args):
    ...
    classes_so_far = dict_keys.pop('classes_so_far', None)
    old_net_state = {k: dict_keys.pop(k) for k in list(dict_keys) if k.startswith('old_net.')}

    model.load_state_dict(dict_keys, strict=False)
    if classes_so_far is not None:
        try:
            model.register_buffer('classes_so_far', classes_so_far)
        except Exception:
            setattr(model, 'classes_so_far', classes_so_far)
    if len(old_net_state) > 0 and hasattr(model, 'old_net'):
        cleaned_state = {k.replace('old_net.', ''): v for k, v in old_net_state.items()}
        if getattr(model, 'old_net', None) is None:
            try:
                model.old_net = copy.deepcopy(model.net)
            except Exception as err:
                logging.warning(f'Could not initialize old_net while loading checkpoint: {err}')
                cleaned_state = None
        if cleaned_state is not None and getattr(model, 'old_net', None) is not None:
            try:
                model.old_net.load_state_dict(cleaned_state, strict=False)
            except Exception as err:
                logging.warning(f'Could not load old_net state dict: {err}')
